...
module.exports = function (rootDirectory, logger, indexDocument, errorDocument, fs, routingRules) {
  ...
  var isMatchedRoutingRule = function (routingRule, keyName) {
    const KeyPrefixEquals = routingRule.Condition && routingRule.Condition.KeyPrefixEquals;

    if (!KeyPrefixEquals) return true;
    return keyName.startsWith(KeyPrefixEquals);
  };
  ...
  return {
    ...
    getObject: function (req, res) {
      ...
      fileStore.getObject(req.bucket, keyName, req.headers.range, function (err, object, data) {
        if (err) {
          var routingRule = routingRules.find(function (routingRule) {
            return isMatchedRoutingRule(routingRule, keyName)
          })

          if (routingRule) {
            var [host, port] = routingRule.Redirect.HostName.split(':');
            host = host || req.hostname;
            var hostname = port ? host + ':' + port : host;
